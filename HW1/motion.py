...
def plot_path(timesteps: np.ndarray, brownian_values: np.ndarray, discretization_step_size: float) -> None:
    plt.figure(figsize=(12,5))
    plt.step(timesteps, brownian_values)
    plt.title(fr'Brownian Motion ($\Delta t = {discretization_step_size}$)')
    plt.xlabel('Time (t)')
    plt.ylabel('B(t)')
    plt.grid(True)
    plt.tight_layout()
